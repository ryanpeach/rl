...
Using a Q Network as a Critic like in A2C_Q_Critic.

Made to look similar to my A2C code for ease of comparison.

Note: Not currently working for whatever reason.
...
def to_onehot(y: int, length: int) -> torch.FloatTensor:
    """
    Takes an integer y and an integer length
    and returns a zero vector of length `length` with a 1 at index `y`
    """
    y_onehot = torch.FloatTensor(length).to(device)
    y_onehot.zero_()
    y_onehot[y] = 1
    return y_onehot
...
def create_worker(gnet_actor: Actor,
    ...
    if int(name[1:]) == 0:
        video_recorder = VideoRecorder(lenv, './output/05_Cartpole_A3C_Q_Critic.mp4', enabled=True)
    else:
        video_recorder = None
    ...
    while global_episode.value < N_ITERS:
        ...
        for _ in count():
            ...
            if total_step % UPDATE_GLOBAL_ITER == 0 or done:
                # sync
                next_policy_dist = lnet_actor(torch.FloatTensor(next_state).to(device))
                ...
                final_value = lnet_critic(next_state, next_policy_dist) if not done else 0
...
if __name__ == "__main__":
    ...
    ax.set_title("Cartpole A3C Q Critic - Rewards over Training")
    ax.plot(results, label="rewards")
    ax.plot(results_ma_, label="moving average")
    fig.savefig('./output/Cartpole_A3C_Q_Critic.png')
    fig.show()
